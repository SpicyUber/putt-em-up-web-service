...
{
    public class SearchMatchesQueryHandler : IRequestHandler<SearchMatchesQuery,MatchPreviewPage>
    {
        private readonly IUnitOfWork uow;
        private readonly IAvatarProvider ap;

        public SearchMatchesQueryHandler(IUnitOfWork uow, IAvatarProvider ap)
        {
            this.uow= uow;
            this.ap= ap;
        }

        public Task<MatchPreviewPage> Handle(SearchMatchesQuery request, CancellationToken cancellationToken)
        {
        ...
        private IQueryable<Domain.Match> FindMatchesAfterDate(DateTime startDate )
        {
            ...
            return uow.MatchRepository.Query().Where((Domain.Match m) => m.StartDate.Date >= startDate.Date && !m.Cancelled);
        ...
        {
            return uow.MatchRepository.Query().Where((Domain.Match m) => m.StartDate.Date.Equals( startDate.Date) && !m.Cancelled);
